...
const SaleEditPage = () => {
    ...
    const onSubmit = async (data) => {
        trackPromise(
            saveSale(data).then((result) => {
                createAlert("success", "Venta actualizada.")
                history.push("/sale")
            }).catch((error) => {
    ...
    const saveSale = async (data) => {
        try {
            ...
            let promiseDelInventory = deleteInventory(saleId)
            let promiseDelClientCommission = deleteClientCommission(saleId)
            let promiseDelVendorCommission = deleteVendorCommission(saleId)
            let promiseDelDelivery = deleteDelivery(saleId)

            await Promise.all([promiseDelInventory, promiseDelClientCommission,
                promiseDelVendorCommission, promiseDelDelivery])
            ...
            db.collection('sale')
                ...
                .then((doc) => {
                    if (doc.exists) {
                        ...
                        for (const property in sale) {
                            let value = {}

                            if (property === "clientId") {
                                if (clientData && clientData.length > 0) {
                                    value = clientData.find(o => o.value === sale[property])
                                    setValue(property, value)
                                }
                            }
                            else if (property === "vendorId") {
                                if (vendorData && vendorData.length > 0) {
                                    value = vendorData.find(o => o.value === sale[property])
                                    setValue(property, value)
                                }
                            }
                            else if (property === "warehouseId") {
                                if (warehouseData && warehouseData.length > 0) {
                                    value = warehouseData.find(o => o.value === sale[property])
                                    setValue(property, value)
                                }
                            }
                            else if (property === "addressIndex") {
                                setAddressIndex(sale[property])
                            }
                            else if (property === "products") {
                                if (productData && productData.length > 0 && priceTypeData && priceTypeData.length > 0 && warehouseInventoryData && warehouseInventoryData.length > 0) {
                                    let products = sale[property]
                                    let productsToLoad = []

                                    products.forEach(p => {
                                        let product = productData.find(pd => pd.value === p.productId)
                                        ...
                                        productsToLoad.push(productToAdd)
                                    })

                                    setValue("products", productsToLoad)
                                }
                            }
                            else if (property === "paymentTypeId") {
                                if (paymentTypeData && paymentTypeData.length > 0) {
                                    value = paymentTypeData.find(o => o.value === sale[property])
                                    setValue(property, value)
                                }
                            }
                            else if (property === "bankAccountId") {
                                if (bankAccountData && bankAccountData.length > 0) {
                                    value = bankAccountData.find(o => o.value === sale[property])
                                    setValue(property, value)
                                }
                            }
                            else if (property === "address") {
                                console.log("ignore ", property)
                            }
                            else if (property === "date") {
                                value = sale[property].toDate()
                                setValue(property, value)
                            }
                            else {
                                setValue(property, sale[property])
                            }
                        }
                    }
                })
                .catch((error) => {
                    console.error("Error retrieving original sale: ", error)
                    createAlert("error", "No se encontro la Venta original.")
                    history.push("/sale")
                }))
