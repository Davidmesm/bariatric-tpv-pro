...
const VendorEditPage = () => {
    ...
    const onSubmit = (data) => {
        trackPromise(
            db.collection("vendor")
                ...
                .update(data)
                .then(() => {
                    console.info("vendor updated.")
                    createAlert("success", "Vendedor actualizado.")
                    history.push("/vendor")
                })
                .catch((error) => {
                    ...
                    setError("save", {
                        type: "firebase-error",
                        message: "Error Desconocido: No se pudo actualizar el vendedor."
                    })
                }))
    }
                ...
                .then(doc => {
                    if (doc.exists) {
                        ...
                        for (const property in vendor) {
                            ...
                            if(property === "bankAccountId")
                            {
                                value = optionsBA ? optionsBA.find(o => o.value === vendor[property]) : {}
                                setValue(property, value);
                            }
                            else
                            {
                                setValue(property, vendor[property]);
                            }
                        }
                    }
                })
                .catch((error) => {
                    console.error("Error retrieving original vendor: ", error)
                    createAlert("error", "No se encontro el Vendedor.")
                    history.push("/vendor")

                }))
    }, [vendorId, setValue, optionsBA, createAlert, history])
    ...
    return (
        <FormPageLayout title="Editar Vendedor">
