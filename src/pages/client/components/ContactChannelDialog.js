...
const ContactChannelDialog = (props) => {
    ...
    const onSubmit = async (data) => {
        ...
        db.collection("contactChannel")
            .add(data)
            .then((doc) => {
                console.info("Contact Channel created: ", doc.id)
                createAlert("success", "Canal de contacto creado.")
                setOpen(false)
                setValue("order", "1")
            })
            .catch((error) => {
                ...
                setError("save", {
                    type: "firebase-error",
                    message: "Error Desconcido: No se pudo guardar el canal de contacto."
                })
            }))
    }
    ...
    return (
        <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
            <form onSubmit={handleSubmit(onSubmit, onError)} id="contact-channel-form">
                <DialogTitle id="form-dialog-title">Agregar Canal de Contacto</DialogTitle>
                <DialogContent>
                    <Box
                        display="flex"
...
export default ContactChannelDialog
